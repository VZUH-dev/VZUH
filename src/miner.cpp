...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VZUHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("VZUHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("VZUHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vzuh-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("VZUHMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running VZUHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadVZUHMiner() exception");
    } catch (...) {
        LogPrintf("ThreadVZUHMiner() exception");
    }

    LogPrintf("ThreadVZUHMiner exiting\n");
}
