...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::VZUH)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("VZUH")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeVZUHAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVZUHAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeVZUHAmount = strAnonymizeVZUHAmount.remove(strAnonymizeVZUHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVZUHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeVZUHAmount * COIN) nMaxToAnonymize = nAnonymizeVZUHAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeVZUHAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeVZUHAmount));
        strAnonymizeVZUHAmount = strAnonymizeVZUHAmount.remove(strAnonymizeVZUHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVZUHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
